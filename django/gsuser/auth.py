...
class OIDCAuthenticationBackend(auth.OIDCAuthenticationBackend):
    ...
    def create_user(self, claims):
        ...
        if len([group for group in claims.get('roles') if group in ["dfour_superadmin","dfour_moderator", "dfour_admin", "dfour_basic"]]) > 0:
            user.is_staff = True
        else:
            user.is_staff = False
        # Check if user is admin and hence should get superuser rights
        if "dfour_superadmin" in claims.get('roles'):
